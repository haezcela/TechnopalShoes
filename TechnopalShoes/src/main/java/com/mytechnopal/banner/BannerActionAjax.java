...
public class BannerActionAjax extends ActionAjaxBase {
	...
	protected void executeLogic(JSONObject jsonObj, DataTable dataTable, String action) {
		if(action.equalsIgnoreCase(DataTable.ACTION_VIEW)) {
			...
			try {
				jsonObj.put(LinkDTO.PAGE_CONTENT, PageUtil.getDataViewPage(sessionInfo, BannerUtil.getDataViewStr(sessionInfo, bannerSelected)));
			...
			try {
				jsonObj.put(LinkDTO.PAGE_CONTENT, PageUtil.getDataEntryPage(sessionInfo, BannerUtil.getDataEntryStr(sessionInfo, banner, uploadedFile)));
			} catch (JSONException e) {
				e.printStackTrace();
			}
